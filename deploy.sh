...
# Get some user input
default_slug="acf-button"
# Can't use the -i flag for read, since that doesn't work for bash 3
printf "1a) WordPress Repo Plugin Slug e.g. acf-button: "
read -e input
PLUGINSLUG="${input:-$default_slug}" #Populate with default if empty
echo

# Set up some default values. Feel free to change these in your own script
CURRENTDIR=`pwd`
default_svnpath="$CURRENTDIR/tmp/$PLUGINSLUG"
default_svnurl="http://plugins.svn.wordpress.org/$PLUGINSLUG"
default_svnuser="circlecube"
default_plugindir="/Users/emullins/www/local.plugins.com/app/public/wp-content/plugins/$PLUGINSLUG"
